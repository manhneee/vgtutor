...
//helpler function để thêm thông báo
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_status'], $_POST['report_id'])) {
    ...
    if (in_array($new_status, $valid_status)) {
        ...
        if ($new_status === 'done') {
            ...
            $user_id = $getUserStmt->fetchColumn();
            if ($user_id) {
                ...
                addNotification($conn, $user_id, $subject, "Your concern regarding \"$subject\" has been addressed by the admin.");
            }
        }
    }
