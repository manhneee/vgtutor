...
if (isset($_SESSION['adminid']) && isset($_SESSION['role'])) {
    if ($_SESSION['role'] == 'Admin') {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'], $_POST['tutorid'], $_POST['courseid'])) {
            ...
            // Lấy tên môn học
            $stmt = $conn->prepare("SELECT course_name FROM course WHERE courseid = ?");
            $stmt->execute([$courseid]);
            $courseName = $stmt->fetchColumn();

            if ($action === 'permit') {
                $msg = "Your offering for course \"$courseName\" has been  ACCEPTED  by admin.";
                $title = "Offering Approved";
            } else {
                $msg = "Your offering for course \"$courseName\" has been  DENIED  by admin.";
                $title = "Offering Denied";
            }
            addNotification(
                $conn,
